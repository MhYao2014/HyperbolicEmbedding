...
namespace fasttext {
    ...
    real DenseMatrix::CosSim(const Vector& vec, int64_t i) const {
        assert(i >= 0);
        assert(i < m_);
        assert(vec.size() == n_);
        real d = 0.0;
...
//    result = fmax(result, 0.01);
    ...
    void DenseMatrix::expMapToRow(fasttext::Vector & grad, int64_t i) {
        assert(i >= 0);
        assert(i < m_);
        assert(grad.size() - 1 == n_);
        real gradLorentzNorm = lorentzNorm(grad);
