...
void Vector::deepcopy(fasttext::Vector &x) {
    assert(size() == x.size());
    for (int64_t i = 0; i < size(); i++) {
        data_[i] = x[i];
    }
...
void Vector::proj(fasttext::Vector &x) {
  assert(x.size() == size());
  real lproduct = lorentzProduct(x);
//  std::cerr << lproduct << std::endl;
  for (int64_t i = 0; i < size(); i++) {
    data_[i] += lproduct * x[i];
  }
}
