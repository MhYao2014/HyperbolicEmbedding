...
int64_t Dictionary::ntokensTree() const {
    return ntokensTree_;
}
...
int32_t Dictionary::readLineMine(
        std::istream& in,
    ...
    // 外面的循环是遍历每行的，遍历完得到一个句子
    bool EOL = false;
    // 当前key对应的value进行初始化
    std::vector<std::string> value;
    while (true) {
        // 因为每行第一个单词为key，所以flag就初始化为true
        bool isKey = true;
        key.clear();
        token.clear();
        while ((c = sb.sbumpc()) != EOF) {
            ...
            if (c == ' ' || c == '\r' || c == '\t' || c == '\v' || c == '\f' || c == '\0'){
                ...
                if (isKey) {
                    ...
                    //如果不是key，那么单词就放入到value中
                    value.push_back(token);
                    token.clear();
                }
            } else if (c == '\n' && !(value.empty())) {
                // 遇到换行符号.并且value不为空，说明读完了一行样本
                if (token != " ") {
                    //如果不是key，那么单词就放入到value中
                    value.push_back(token);
                    token.clear();
                ...
                // 遇到换行符号，并且value为空，说明一句话已经读完，设立flag
                EOL = true;
                // 跳出读取当前单词的循环
                break;
            }
        ...
        // 一行样本读完，将{key, value}对存入字典中，如果当前样本没有被抛弃的话
        if (!key.empty()) {
...
void Dictionary::readFromFileTree(std::istream & in) {
    std::unordered_map<std::string, std::vector<std::string>> treedict;
    int32_t num;
    bool NotDone = true;
    while (NotDone) {
        num = readLineMine(in, treedict);
        ntokensTree_ += num;
        if (in.eof()) {
            NotDone = false;
        }
        if (ntokensTree_ % 1000000 == 0 && args_->verbose > 1) {
            std::cerr << "\rRead " << ntokensTree_  << " words" << std::flush;
        }
    }
...
void Dictionary::reset(std::istream& in) const {
  if (in.eof()) {
    ...
    int32_t Dictionary::getLineMine(
            std::istream& in,
            std::unordered_map<std::string, std::vector<std::string>>& treedict,
