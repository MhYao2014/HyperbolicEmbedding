...
//      return std::make_shared<OneVsAllLoss>(output);
    ...
    void FastText::printInfoMine(real progress,real progressTree, real loss, real lossTree, std::ostream& log_stream) {
      ...
      double lrTree = args_->lr * sqrt((1.0 - progressTree)*(1.0 - progressTree));
      double wst = 0, wstTree = 0;
      ...
      log_stream << "SkipProgress: ";
      log_stream << std::setprecision(1) << std::setw(3) << progress << "% ";
      log_stream << " words/sec/thread: " << std::setw(3) << int64_t(wst);
      log_stream << " lr: " << std::setw(7) << std::setprecision(6) << lr;
      log_stream << " loss: " << std::setw(3) << std::setprecision(6) << loss;
      log_stream << " ETA: " << std::setw(1) << etah;
      log_stream << "h" << std::setw(1) << etam << "m";
//      log_stream << std::flush;
      log_stream << "\n";
      log_stream << std::fixed;
      log_stream << "TreeProgress: " ;
      log_stream << std::setprecision(1) << std::setw(3) << progressTree << "% ";
      log_stream << " words/sec/thread: " << std::setw(3) << int64_t(wstTree);
      log_stream << " lr: " << std::setw(7) << std::setprecision(6) << lrTree;
      log_stream << " loss: " << std::setw(3) << std::setprecision(6) << lossTree;
      log_stream << " ETA: " << std::setw(1) << etahTree;
      log_stream << "h" << std::setw(1) << etamTree << "m";
//      log_stream << std::flush;
    }
...
// depreacted, use getAnalogies instead
    ...
    void FastText::startThreads() {
      ...
      while (true) {
        if (tokenCount_ > args_->epoch * ntokens && tokenCountTree_ > args_->epochTree * ntokensTree) {
          break;
        }
        ...
        std::cerr << "\r";
        if (args_->loss == loss_name::uns) {
          real progressTree = real(tokenCountTree_) / (args_->epochTree * ntokensTree);
          std::cerr << "\r";
          printInfoMine(progress, progressTree, loss_, lossTree_, std::cerr);
        } else {
          printInfoMine(progress, 100, loss_, 0, std::cerr);
        }
      }
      for (int32_t i = 0; i < args_->thread; i++) {
        threads[i].join();
    ...
    void FastText::trainThread(int32_t threadId) {
      ...
      while ( true ) {
        ...
        real progressTree = real(tokenCountTree_) / (args_->epochTree * ntokensTree);
        real lrTree = fmax(args_->lrTree * (1.0 - progressTree), 0);
        if (args_->model == model_name::sg) {
          ...
          if (args_->loss == loss_name::uns) {
            if (tokenCountTree_ < args_->epochTree * ntokensTree) {
              localTokenCountTree += dict_->getLineMine(ifsTree, treedict, state.rng);
        ...
        if (args_->loss == loss_name::uns && localTokenCountTree > args_->lrUpdateRate && tokenCountTree_ < args_->epochTree * ntokensTree) {
